 ...
 */
        ...
        var rootjQuery, rquickExpr = /^(?:\s*(<[\w\W]+>)[^>]*|#([\w-]+))$/, init = jQuery.fn.init = function(selector, context, root) {
            ...
            if (typeof selector === "string") {
                ...
                this.length = 1;
                return this;
            } else if (jQuery.isFunction(selector)) return root.ready !== void 0 ? root.ready(selector) : selector(jQuery);
        ...
        function adoptValue(value, resolve, reject, noValue) {
            var method;
            try {
                if (value && jQuery.isFunction(method = value.promise)) method.call(value).done(resolve).fail(reject); else if (value && jQuery.isFunction(method = value.then)) method.call(value, resolve, reject); else resolve.apply(void 0, [ value ].slice(noValue));
            } catch (value) {
                        ...
                        function resolve(depth, deferred, handler, special) {
                            return function() {
                                var that = this, args = arguments, mightThrow = function() {
                                    ...
                                    if (jQuery.isFunction(then)) if (special) then.call(returned, resolve(maxDepth, deferred, Identity, special), resolve(maxDepth, deferred, Thrower, special)); else {
                                        maxDepth++;
        ...
        Data.prototype = {
            ...
            get: function(owner, key) {
                return key === void 0 ? this.cache(owner) : owner[this.expando] && owner[this.expando][jQuery.camelCase(key)];
            },
            ...
            remove: function(owner, key) {
                ...
                if (key !== void 0) {
                    if (Array.isArray(key)) key = key.map(jQuery.camelCase); else {
        ...
        jQuery.event = {
            ...
            dispatch: function(nativeEvent) {
                ...
                while ((matched = handlerQueue[i++]) && !event.isPropagationStopped()) {
                    ...
                    j = 0;
                    while ((handleObj = matched.handlers[j++]) && !event.isImmediatePropagationStopped()) if (!event.rnamespace || event.rnamespace.test(handleObj.namespace)) {
                ...
                if (first || ignored) {
                    ...
                    if (hasScripts) {
                        ...
                        for (i = 0; i < hasScripts; i++) {
                            node = scripts[i];
                            if (rscriptType.test(node.type || "") && !dataPriv.access(node, "globalEval") && jQuery.contains(doc, node)) if (node.src) {
        ...
        jQuery.extend(jQuery.event, {
            trigger: function(event, data, elem, onlyHandlers) {
                ...
                event.type = type;
                if (!onlyHandlers && !event.isDefaultPrevented()) if ((!special._default || special._default.apply(eventPath.pop(), data) === false) && acceptData(elem)) if (ontype && jQuery.isFunction(elem[type]) && !jQuery.isWindow(elem)) {
                    tmp = elem[ontype];
        ...
        jQuery.each({
            ...
            jQuery.fn[method] = function(val) {
                return access(this, (function(elem, method, val) {
                    var win;
                    if (jQuery.isWindow(elem)) win = elem; else if (elem.nodeType === 9) win = elem.defaultView;
    ...
    $(document).ready((function() {
        $(".block-foto__row").slick({
            ...
            slidesToShow: 4,
            infinity: true,
            asNavFor: ".block-foto-sldr",
            focusOnSelect: true
        });
        $(".block-foto-sldr").slick({
            arrows: false,
            dots: false,
            fade: true,
            slidesToScroll: 1,
            infinity: true,
            asNavFor: ".block-foto__row",
            draggable: false
        });
    }));
    $(document).ready((function() {
    ...
    $(document).ready((function() {
        var childElements = $(".links");
        $(window).on("resize", (function() {
            if (window.innerWidth <= 767) childElements.each((function() {
                var $this = $(this);
                if (!$this.data("original-parent")) $this.data("original-parent", $this.parent());
                $this.appendTo($this.closest(".blog-section"));
            })); else childElements.each((function() {
                var $this = $(this);
    ...
    (function($) {
        $((function() {
            $("ul.tb ").each((function(i) {
                ...
                if (storage) $(this).find("li").removeClass("choice").eq(storage).addClass("choice").closest("div.blocktb").find("div.tbcon").removeClass("choice").eq(storage).addClass("choice");
            }));
            $("ul.tb ").on("click", "li:not(.choice)", (function() {
                $(this).addClass("choice").siblings().removeClass("choice").closest("div.blocktb").find("div.tbcon").removeClass("choice").eq($(this).index()).addClass("choice");
